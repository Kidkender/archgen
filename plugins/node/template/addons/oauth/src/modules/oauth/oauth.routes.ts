...
export default async function oauthRoutes(app: FastifyInstance) {
  ...
  app.get("/google", async (req: FastifyRequest, reply: FastifyReply) => {
    // @fastify/oauth2 v8: generateAuthorizationUri requires (request, reply)
    const url = await (app as any).googleOAuth2.generateAuthorizationUri(req, reply);
    return reply.redirect(url);
  });

  app.get("/google/callback", async (req, reply) => {
    const token = await (app as any).googleOAuth2.getAccessTokenFromAuthorizationCodeFlow(req, reply);
  ...
  app.get("/github", async (req: FastifyRequest, reply: FastifyReply) => {
    // @fastify/oauth2 v8: generateAuthorizationUri requires (request, reply)
    const url = await (app as any).githubOAuth2.generateAuthorizationUri(req, reply);
    return reply.redirect(url);
  });

  app.get("/github/callback", async (req, reply) => {
    const token = await (app as any).githubOAuth2.getAccessTokenFromAuthorizationCodeFlow(req, reply);
