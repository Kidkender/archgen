...
export const addCommand = new Command("add")
  .argument("<addon>", "Addon to inject (docker|testing|ci)")
  ...
  .action(async (addon: string, options: { dryRun: boolean }) => {
    ...
    if (!language) {
      ...
      logger.info("Run this command from a project root containing package.json (Node/Fastify) or pyproject.toml / main.py (Python).");
      process.exit(1);
    }
